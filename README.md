...
- 傅里叶级数
  - 对于一组标准正交基qn，他们可以在任意的x向量线性组合下构成空间内部任意的向量v，那么每一个系数xn=qnTv。
  - 向量有向量的内积，则任意两个函数的内积同理
  - 这样对于一个傅里叶级数，它的每个系数的求法就和上述xn的求法相同了，傅里叶把函数看成了无限的矩阵。

### 第26课 对称矩阵及正定性

- 对称阵
  - 对称阵的特征值是实数，特征向量中能挑选出来一组相互垂直的
  - 通常 A=SΛS−1，若A为对称的，则S中的特征向量相互垂直，且取标准正交向量，则A=QΛQ−1
...
- 快速傅里叶变换FFT
  - 傅里叶矩阵的每一列都是正交的，除以模长后得到标准正交矩阵Q，根据性质F−1n=FHn，方便的求自身的逆
...
- 图像压缩
  ...
  - 好基要满足的条件：
    - 1. 计算块，求逆快，一般是用正交矩阵，转置直接得逆；
    - 2，少量向量就能接近原信号
  - 设原向量p，新基矩阵为W，则p=Wc，c=W-1p，就是新的线性组合系数
